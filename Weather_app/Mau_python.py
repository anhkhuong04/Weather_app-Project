...
# Image
new = ImageTk.PhotoImage(file="Images/logo2.png")
...
if int((dt.strftime('%I'))) >= 8 & int((dt.strftime('%I'))) <= 5:
	img = ImageTk.PhotoImage(file="Images/moon.png")
	panel = Label(root, image=img)
	panel.place(x=210, y=200)
else:
	img = ImageTk.PhotoImage(file="Images/sun.png")
	panel = Label(root, image=img)
	panel.place(x=210, y=200)
...
def city_name():
	...
	api_request = requests.get("https://api.openweathermap.org/data/2.5/weather?q="
							+ city_entry.get() + "&units=metric&appid=97a310e1694d1983ac5b6d2c58289650")
